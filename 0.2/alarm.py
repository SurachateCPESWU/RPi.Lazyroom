...
if(NUM_ALARM>=1):
    ...
    PIN_POWER=Get_POWER()
    PIN_LIGHT=23
    TIME_A = int(TIME_ALARM[0][0])
    #################################
    print TIME_ALARM[0][0]
    while 1:
       #########COMPUTER STATUS###########
       ...
       if int(time.strftime("%H%M")) == TIME_A and  com_status == 0 :
        print("START")
        os.system("gpio write %d 0"%PIN_POWER)
        time.sleep(1)
        os.system("gpio write %d 1"%PIN_POWER)
        time.sleep(20)
        os.system("cvlc /home/pi/Desktop/lazyroom/media/music /home/pi/Desktop/lazyroom/media/go.mp4 vlc://quit")
